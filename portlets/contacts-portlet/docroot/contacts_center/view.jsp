...
<c:choose>
	...
	<c:otherwise>
						...
						%>
							...
							<div class="lfr-contact">
								<div class="lfr-contact-checkbox">
									<input label="" class="contact-ids" name="contact-ids-<%= user2.getUserId() %>" type="checkbox" value="<%= user2.getUserId() %>" />
								</div>
		...
		<liferay-portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>" var="toolbarURL">
			<portlet:param name="mvcPath" value="/contacts_center/contacts_center_toolbar.jsp" />
		</liferay-portlet:renderURL>
		...
		<aui:script use="aui-base,aui-io,datatype-number">
			...
			contactsResult.delegate(
				'click',
				function(event) {
					Liferay.Contacts.clearContactResult();
						...
						uri,
						{
							after: {
								success: function(event, id, obj) {
									...
									var contactSummary = content.one('#<portlet:namespace />userSummary');
									...
									if (contactSummary) {
										...
										contactSummaryContainer.plug(A.Plugin.ParseContent);
									...
									if (userToolbar) {
										var userToolbarContainer = A.one('#toolbarButtons');

										userToolbarContainer.empty();

										userToolbarContainer.plug(A.Plugin.ParseContent);
					...
					if (checkBox.get('checked')) {
						A.io.request(
							'<portlet:resourceURL id="getContact"><portlet:param name="portletResource" value="<%= portletResource %>" /></portlet:resourceURL>',
							{
								after: {
									success: function(event, id, obj) {
										Liferay.Contacts.addContactResult(this.get('responseData'), '<%= toolbarURL %>');
									}
								},
								data: {
									userId: userId
						...
						Liferay.Contacts.deleteContactResult(userId);
					}
				},
				'.contact-ids'
			);
